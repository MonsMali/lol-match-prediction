...
class InteractiveLoLPredictor:
    ...
    def __init__(self, model_path=None):
        ...
        # Robustness components (initialized after model loading)
        ...
        self.setup_robustness_components()
                ...
                if os.path.exists(path):
                    ...
                    if isinstance(deployment_package, dict) and 'model' in deployment_package:
                        ...
                        if self.scaler is not None:
                            ...
                            print(f"    No scaler in deployment package, trying separate files...")
    ...
    def setup_feature_engineering(self):
        ...
        try:
            ...
            data_paths = [
                os.path.join(project_root, "data", "processed", "complete_target_leagues_dataset.csv"),  # New organized structure
                os.path.join(project_root, "data", "complete_target_leagues_dataset.csv"),  # Legacy location
                "../../data/processed/complete_target_leagues_dataset.csv",
                "../../data/complete_target_leagues_dataset.csv",
                "../data/complete_target_leagues_dataset.csv",
                "Dataset collection/complete_target_leagues_dataset.csv",  # Legacy paths
                "../Dataset collection/complete_target_leagues_dataset.csv"
            ]
    ...
    def load_champion_database(self):
        ...
        try:
            ...
            print(f"    Could not load champion database: {e}")
            print(f"   Using default champion pool")
            self.valid_champions = set(self.champion_pool)

    def setup_robustness_components(self):
        """Initialize robustness components for enhanced predictions."""
        print(f"\n   SETTING UP ROBUSTNESS COMPONENTS")

        try:
            # Setup edge case handler from training data
            if self.feature_engineering and hasattr(self.feature_engineering, 'df'):
                df = self.feature_engineering.df
                self.edge_case_handler = EdgeCaseHandler.from_training_data(df)
                stats = self.edge_case_handler.get_summary_stats()
                print(f"   Edge case handler ready: {stats['known_champions']} champions, {stats['known_teams']} teams")
            else:
                self.edge_case_handler = EdgeCaseHandler()
                print(f"   Edge case handler ready (empty)")

            # Note: Confidence estimator and model explainer are initialized
            # on-demand during predictions to avoid loading large calibration data
            print(f"   Confidence estimation: available on-demand")
            print(f"   Model explanations: available on-demand")

        except Exception as e:
            print(f"   Warning: Robustness setup partial: {e}")
            self.edge_case_handler = EdgeCaseHandler()
    ...
    def get_champion_input_with_validation(self, prompt, suggestions=None, used_champions=None):
        ...
        while True:
            ...
            if not champion_input:
                print(f"    Please enter a champion name or command")
                continue
            ...
            if validated_champion:
                ...
                if choice == 'f':
                    ...
                    print(f"    Using '{formatted_name}' (may cause prediction errors)")
    ...
    def _execute_draft(self, teams):
        ...
        for i, role in enumerate(roles):
            ...
            while True:
                ...
                if not champion_input:
                    print(f"    Please select a champion")
                    continue
    ...
    def predict_match(self, match_data):
        """Make prediction for the match with enhanced robustness features."""
        print(f"\nMAKING PREDICTION")
        print("=" * 50)
        ...
        try:
            ...
            if self.edge_case_handler:
                edge_case_result = self.edge_case_handler.detect(match_data)
                if edge_case_result.get('has_edge_cases'):
                    print("\nEdge Case Detection:")
                    for warning in edge_case_result.get('warnings', []):
                        print(f"   Warning: {warning}")
                    confidence_penalty = edge_case_result.get('confidence_penalty', 0)

            # Step 2: Prepare features for both sides
            # Blue side prediction
            blue_data = match_data.copy()
            blue_data['team'] = blue_data['blue_team']
            blue_data['side'] = 'Blue'
            blue_data['result'] = 0  # Placeholder
            ...
            if self.scaler:
                ...
                blue_scaled = blue_final.values
                red_scaled = red_final.values
            ...
            blue_win_prob = blue_pred[1]
            red_win_prob = red_pred[1]

            # Average both perspectives for final prediction
            avg_blue_win = (blue_win_prob + (1 - red_win_prob)) / 2
            avg_red_win = 1 - avg_blue_win

            # Step 5: Calculate confidence with edge case penalty
            winner = match_data['blue_team'] if avg_blue_win > 0.5 else match_data['red_team']
            raw_confidence = max(avg_blue_win, avg_red_win)

            # Apply edge case penalty to confidence
            adjusted_confidence = raw_confidence * (1 - confidence_penalty)
            ...
            margin = 0.05 + (0.10 * (1 - abs(avg_blue_win - 0.5) * 2))  # Wider near 0.5
            lower_bound = max(0, avg_blue_win - margin)
            upper_bound = min(1, avg_blue_win + margin)

            # Step 6: Get explanation (simplified - top features from feature importance)
            top_factors = self._get_top_prediction_factors(blue_final)

            # Step 7: Display enhanced results
            print(f"\n{'='*50}")
            print("MATCH PREDICTION RESULTS")
            print(f"{'='*50}")
            print(f"Blue Team: {match_data['blue_team']}")
            print(f"Red Team: {match_data['red_team']}")
            print(f"\nPrediction: {winner} wins")
            print(f"Probability: {avg_blue_win:.1%} (Blue) vs {avg_red_win:.1%} (Red)")
            print(f"Confidence: {confidence_level} ({adjusted_confidence:.1%})")
            print(f"95% CI: [{lower_bound:.1%}, {upper_bound:.1%}]")

            # Display top factors
            if top_factors:
                print(f"\nTop Factors:")
                winner_is_blue = avg_blue_win > 0.5
                for factor in top_factors[:5]:
                    impact = factor.get('impact', 'neutral')
                    name = factor.get('feature', 'unknown')
                    value = factor.get('value', 0)
                    sign = '+' if (impact == 'positive' and winner_is_blue) or (impact == 'negative' and not winner_is_blue) else '-'
                    print(f"   {sign} {name}: {value:.3f}")

            # Display edge case warning if any
            if edge_case_result and edge_case_result.get('has_edge_cases'):
                print(f"\nEdge Case Warning: Confidence reduced by {confidence_penalty:.0%}")
            ...
            return {
                ...
                'edge_case_warning': edge_case_result.get('warnings') if edge_case_result else None
            }
    ...
    def _get_top_prediction_factors(self, features_df, top_n=10):
        ...
        try:
            ...
            # Combine importance with feature values
            factors = []
            for i, (name, importance) in enumerate(zip(feature_names, importances)):
                if i < len(feature_values):
                    value = feature_values[i]
                    # Determine impact direction based on value and importance
    ...
    def apply_prediction_time_encoding(self, advanced_features_df):
        ...
        try:
            ...
            encoders = {}
            print(f"    No pre-trained encoders found, using fallback values")
